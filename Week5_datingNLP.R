...
pdf( file.path( 'dating', 'Week5_dating.pdf') )
...
# give information about the length of the texts
...
ggplot(profiles, aes(x = TextLength, fill = sex)) +
  ...
  labs(y = "Text Count", x = "Length of Text",
       title = "Distribution of Text Lengths with by gender")
...
#dfm <- dfm_trim(dfm, min_docfreq = 10, min_termfreq = 20, verbose = TRUE)

dim(dfm)


#dfm <- dfm[1:5000,1:5000]
...
#mode(all.tokens.matrix) <- 'integer'


#library(textir)
#all.tokens.dfm.new <- tfidf(all.tokens.dfm)
...
pdf( file.path( 'dating', 'Week5_datingNLP.pdf') ,width = 20, height = 20)
